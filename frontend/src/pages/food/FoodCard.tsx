...
import { FoodCard, Restaurant } from '../../components/FoodCard'; // Import your component
...
export default function FoodExplore() {
  ...
  const carouselSlides = useMemo(() => {
    ...
    // Ensure rating is treated as number for sorting
    return [...restaurants].sort((a, b) => (b.rating || 0) - (a.rating || 0)).slice(0, 5);
  }, [restaurants]);
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '32px' }}>
              {paginatedRestaurants.map((place) => (
                // Use the imported FoodCard component
                <FoodCard key={place.id} restaurant={place} />
              ))}
            </div>
