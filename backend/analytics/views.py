...
# Legacy (analytics.models_old) — used by some read-only/example endpoints
from .models_old import POI, PostRaw, PostClean, SentimentTopic

# New (analytics.models) — your live Place/SocialPost data
...
class SentimentSummaryView(APIView):
    ...
    -> {
         "positive_pct": 68, "neutral_pct": 23, "negative_pct": 9,
         "positive": 312000, "neutral": 105000, "negative": 41000,
         "mentions": 458000
       }
    """
    def get(self, request):
...
def destinations_top(request):
    ...
    -> [{ name, visitors, posts, rating, change, trend }]
    """
    start, end = _range_from_or_date_params(request)
    if not start or not end:
        return Response({"detail": "Invalid date range"}, status=400)
...
def destinations_undervisited(request):
    ...
    -> [{ name, visitors, posts, change, trend }]
    """
    start, end = _range_from_or_date_params(request)
    if not start or not end:
        return Response({"detail": "Invalid date range"}, status=400)
...
def social_trends(request):
    ...
    -> [{ date:'YYYY-MM', likes, comments, shares }]
    Groups by day (you can switch to month later).
    """
    start, end = _range_from_or_date_params(request)
    if not start or not end:
        return Response({"detail": "Invalid date range"}, status=400)
...
def social_sentiment_summary(request):
    ...
    GET /api/social/sentiment/summary?from=&to=
    -> {"positive_pct": 68, "neutral_pct": 23, "negative_pct": 9,
...
def social_sentiment_by_category(request):
    """
    GET /api/social/sentiment/by-category?from=&to=
...
def social_keywords(request):
    """
    GET /api/social/keywords?from=&to=&limit=12
    -> [{ term, count }]
    """
    start, end = _range_from_or_date_params(request)
    if not start or not end:
