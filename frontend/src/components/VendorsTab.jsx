...
export default function VendorsTab({ dateFrom, dateTo }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/tabs/vendors?${p.toString()}`)
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    const p = new URLSearchParams({ date_from: dateFrom, date_to: dateTo });
    fetch(`${API_BASE}/tabs/vendors/${active.id}?${p.toString()}`)
      .then((r) => r.json())
      .then((d) => setDetail(d))
  ...
  return (
    <div className="card" style={{ textAlign: "left" }}>
      <div className="card-title">Food Vendors</div>
      {!items.length ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 12 }}>
          ...
          <div>
            {!detail ? (
              <div style={{ color: "#6b7280", fontSize: 12 }}>Select a vendor to view details.</div>
            ) : (
              <div>
