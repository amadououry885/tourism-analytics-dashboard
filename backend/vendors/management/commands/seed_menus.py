"""
Django management command to seed menu items for restaurants
Usage: python manage.py seed_menus
"""
import random
...
class Command(BaseCommand):
    help = 'Seed menu items for all restaurants'

    def get_menu_for_vendor(self, vendor):
    ...
    def handle(self, *args, **options):
        self.stdout.write('🍽️  Seeding restaurant menus...')
        ...
        for vendor in vendors:
            ...
            menu_items = self.get_menu_for_vendor(vendor)
            ...
            self.stdout.write(f'✅ Added {len(menu_items)} menu items for {vendor.name}')
        
        self.stdout.write('=' * 50)
        self.stdout.write(self.style.SUCCESS(f'✅ Created {created_count} new menu items'))
        self.stdout.write(f'⏭️  Skipped {skipped_count} vendors (already had menus)')
